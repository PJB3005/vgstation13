...
/datum/surgery_step/glue_bone/can_use(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
	return (affected.open >= 2 || (target.species.flags & NO_SKIN)) && affected.stage == 0

/*/datum/surgery_step/glue_bone/can_operate(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
...
/datum/surgery_step/glue_bone/begin_step(mob/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
	if (affected.stage == 0)
...
/datum/surgery_step/glue_bone/end_step(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
...
/datum/surgery_step/glue_bone/fail_step(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
...
/datum/surgery_step/set_bone/can_use(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
	return affected.name != "head" && (affected.open >= 2 || (target.species.flags & NO_SKIN)) && affected.stage == 1

/*/datum/surgery_step/set_bone/can_operate(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
	if(affected.brute_dam >= affected.min_broken_damage * config.organ_health_multiplier)
		to_chat(user, "<span class='warning'>The [affected.display_name] is far too damaged to operate on the fracture. Fix some of the damage first.</span>")
		return 0
...
/datum/surgery_step/set_bone/begin_step(mob/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
...
/datum/surgery_step/set_bone/end_step(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
...
/datum/surgery_step/set_bone/fail_step(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
...
/datum/surgery_step/mend_skull/can_use(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
	return affected.name == "head" && (affected.open >= 2 || (target.species.flags & NO_SKIN))&& affected.stage == 1

/*/datum/surgery_step/mend_skull/can_operate(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
	if(affected.brute_dam >= affected.min_broken_damage * config.organ_health_multiplier)
		to_chat(user, "<span class='warning'>The [affected.display_name] is far too damaged to operate on the fracture. Fix some of the damage first.</span>")
		return 0
...
/datum/surgery_step/mend_skull/begin_step(mob/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
...
/datum/surgery_step/mend_skull/end_step(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
...
/datum/surgery_step/mend_skull/fail_step(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
...
/datum/surgery_step/finish_bone/can_use(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
	return (affected.open >= 2 || (target.species.flags & NO_SKIN)) && affected.stage == 2

/*/datum/surgery_step/finish_bone/can_operate(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
	if(affected.brute_dam >= affected.min_broken_damage * config.organ_health_multiplier)
		to_chat(user, "<span class='warning'>The [affected.display_name] is far too damaged to operate on the fracture. Fix some of the damage first.</span>")
		return 0
...
/datum/surgery_step/finish_bone/begin_step(mob/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
...
/datum/surgery_step/finish_bone/end_step(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
...
/datum/surgery_step/finish_bone/fail_step(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
	user.visible_message("<span class='warning'>[user]'s hand slips, smearing [tool] in the incision in [target]'s [affected.display_name]!</span>" , \
	"<span class='warning'>Your hand slips, smearing [tool] in the incision in [target]'s [affected.display_name]!</span>")
