...
/datum/surgery_step/limb/cut/begin_step(mob/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
...
/datum/surgery_step/limb/cut/end_step(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
...
/datum/surgery_step/limb/cut/fail_step(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
	if (affected.parent)
...
/datum/surgery_step/limb/mend/can_use(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
	return ..() && affected.status & ORGAN_CUT_AWAY

/datum/surgery_step/limb/mend/begin_step(mob/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
...
/datum/surgery_step/limb/mend/end_step(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
...
/datum/surgery_step/limb/mend/fail_step(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
	if (affected.parent)
		affected = affected.parent
...
/datum/surgery_step/limb/prepare/can_use(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
	return ..() && affected.open == 3

/datum/surgery_step/limb/prepare/begin_step(mob/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
...
/datum/surgery_step/limb/prepare/end_step(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
...
/datum/surgery_step/limb/prepare/fail_step(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
	if (affected.parent)
		affected = affected.parent
...
/datum/surgery_step/limb/attach/can_use(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
...
/datum/surgery_step/limb/attach/begin_step(mob/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
...
/datum/surgery_step/limb/attach/end_step(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
...
/datum/surgery_step/limb/attach/fail_step(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
...
/datum/surgery_step/limb/attach_plank/can_use(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
	return ..() && affected.status & ORGAN_ATTACHABLE

/datum/surgery_step/limb/attach_plank/begin_step(mob/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
	user.visible_message("[user] starts attaching [tool] where [target]'s [affected.display_name] used to be.", \
	"You start attaching [tool] where [target]'s [affected.display_name] used to be.")

/datum/surgery_step/limb/attach_plank/end_step(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
	user.visible_message("<span class='notice'>[user] has attached [tool] where [target]'s [affected.display_name] used to be.</span>",	\
	"<span class='notice'>You have attached [tool] where [target]'s [affected.display_name] used to be.</span>")
	affected.peggify()
...
/datum/surgery_step/limb/attach_plank/fail_step(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
	user.visible_message("<span class='warning'>[user]'s hand slips, damaging connectors on [target]'s [affected.display_name]!</span>", \
	"<span class='warning'>Your hand slips, damaging connectors on [target]'s [affected.display_name]!</span>")
	target.apply_damage(10, BRUTE, affected)
...
/datum/surgery_step/limb/attach_flesh/can_use(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
...
/datum/surgery_step/limb/attach_flesh/begin_step(mob/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
	user.visible_message("[user] starts attaching [tool] where [target]'s [affected.display_name] used to be.", \
	"You start attaching [tool] where [target]'s [affected.display_name] used to be.")

/datum/surgery_step/limb/attach_flesh/end_step(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
	user.visible_message("<span class='notice'>[user] has attached [tool] where [target]'s [affected.display_name] used to be.</span>",	\
	"<span class='notice'>You have attached [tool] where [target]'s [affected.display_name] used to be.</span>")
	affected.fleshify()
...
/datum/surgery_step/limb/attach_flesh/fail_step(mob/living/user, mob/living/carbon/humanoid/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/affected = target.get_organ(target_zone)
	user.visible_message("<span class='warning'>[user]'s hand slips, damaging connectors on [target]'s [affected.display_name]!</span>", \
	"<span class='warning'>Your hand slips, damaging connectors on [target]'s [affected.display_name]!</span>")
	target.apply_damage(10, BRUTE, affected)
