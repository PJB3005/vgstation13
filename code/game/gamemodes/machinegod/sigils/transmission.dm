...
/obj/effect/sigil/transmission/activation(var/mob/M as mob)
	...
	if(iscultist(M))
		if(electrocute_mob(M, get_area(src), src, 1))
			to_chat(M, "<span class='sinister'>\"Watch your step, dog.\"</span>")
		else
			to_chat(M, "<span class='warning'>The sigil lights up, but nothing happens...</span>")
	else
		if(electrocute_mob(M, get_area(src), src, 0.5))
			to_chat(M, "<span class='danger'>You are suddenly electrocuted!</span>")
		else
			to_chat(M, "<span class='warning'>The sigil lights up, but nothing happens...</span>")
