...
/datum/game_mode/machinegod/announce()
	to_chat(world, "<B>The current game mode is - Machinegod!</B>")
...
/datum/game_mode/proc/remove_clockcultist(var/datum/mind/clock_mind, var/show_message = 1, var/log = 1)
	if(clock_mind in clockcult)
		...
		clockcult -= clock_mind
		to_chat(clock_mind.current, "<span class='danger'>Thoughts of the Justiciar and his eternal prison fade from your mind. The sounds of celestial clockwork fall silent, and become nothing more than fleeting memories.</span>")
		clock_mind.memory = ""
...
/datum/game_mode/machinegod/proc/memoize_clockcult_objectives(var/datum/mind/clockcult_mind)
	to_chat(clockcult_mind.current, "Use Geis capacitors with your slab to convert others.")
	...
	/*for(var/obj_count = 1,obj_count <= objectives.len,obj_count++)
		...
		switch(objectives[obj_count])
			if("convert")
				explanation = ""
		to_chat(clockcult_mind.current, "<B>Objective #[obj_count]</B>: [explanation]")
