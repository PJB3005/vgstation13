...
/obj/machinery/maniamotor/process()
	...
	for(var/mob/living/L in range(src, 12))
		...
		for(var/turf/T in range(1, L)) //null rod blocks the motor's effects
			if(findNullRod(T))
				if(prob(5))
					to_chat(L, "<span class='info'>The power of the null rod shields your mind from evil.</span>")
				continue
		...
		if(src in view(L)) //If you see it, get ready for brainfuck
			if(prob(6)) to_chat(L, "<span class='sinister'>Your sanity rapidly deteriorates as you look at the [src].</span>")
		...
		if(prob(4))
			to_chat(L, "<span class='sinister'>A dark presence weighs on your mind.</span>")
...
/obj/machinery/maniamotor/proc/bad_event(var/mob/living/L as mob)
	...
	to_chat(L, "<span class='sinister'>Your sanity rapidly deteriorates.</span>")
...
/obj/machinery/maniamotor/examine(mob/user)
	..()
	if(!isclockcult(user))
		to_chat(user, "<span class='sinister'>You hear incessant cackling.</span>")

/obj/machinery/maniamotor/attack_hand(mob/user as mob)
	if(!isclockcult(user) && !iscultist(user))
		to_chat(user, "<span class='warning'>You're unsure of how to operate this device.</span>")
		return

	if(!state)
		to_chat(user, "<span class='warning'>The motor needs to be bolted down first before you activate it!</span>")
	...
	for(var/obj/machinery/maniamotor/MM in range(src, 12))
		...
		to_chat(user, "<span class='sinister'>Another mania motor is already installed nearby. It's unsafe to try and activate these too close to each other!</span>")
		to_chat(user, "<span class='info'>The nearest motor is [get_dist(src, MM)] meters away from your motor.</span>")
		return
