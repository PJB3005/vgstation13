...
/datum/clockcult_power/sentinels_comprimise/activate(var/mob/user, var/obj/item/clockslab/C, var/list/participants)
	...
	if (!possible_cultists.len) //Nobody found.
		to_chat(user, "<span class='clockwork'>You need to stand close to someone to heal!</span>")
		return

	var/mob/living/selected = input(user, "Who do you want do heal?", "Sentinel's Comprimise") as null | mob in possible_cultists
	if (!selected || !isclockcult(selected) || selected.isDead() || !(selected in range(8, user)) || user.incapacitated())
		return

	var/damage = selected.getBruteLoss() + selected.getFireLoss()
	if (damage <= 0)
		to_chat(user, "<span class='clockwork'>[selected] is not injured!</span>")
		return
	...
	*/
	...
	to_chat(user, "<span class='clockwork'>You mend [selected]'s wounds.</span>")
	to_chat(selected, "<span class='clockwork'>[user] painfully mends your wounds, and a strong desire to vomit arises.</span>")

	#warn TODO: Maybe make this scale with toxin amount?
	if (prob(25))
		sleep(2 SECONDS)
		if (ishuman(selected))
			var/mob/living/carbon/human/H = selected
		...
		to_chat(selected, "<span class='warning'>...and so, you do.</span>")
