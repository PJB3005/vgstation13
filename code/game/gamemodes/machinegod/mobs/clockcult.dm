	...
	if(message_mode == MODE_HEADSET && clockcult_chat_check(0))
		...
		for(var/mob/M in mob_list)
			if(M.clockcult_chat_check(2) /*receiving check*/ || ((M in dead_mob_list) && !istype(M, /mob/new_player)))
				to_chat(M, "<span class='clockcult'><b>[src.name]:</b> [message]</span>")
		return 1
...
/mob/living/simple_animal/clockcult/bound_revenant/Life()
	...
	if(contractor.current.stat == UNCONSCIOUS && !contractor_dying)
		to_chat(src, "<span class='sinister'>You feel the bond between you and [contractor.current] grow weak.</span>")
		contractor_dying = 1
...
/mob/living/simple_animal/bound_revenant/verb/toggle_mastery()
	...
	to_chat(src, "<span class='[cmtype ? bnotice : danger]'>You switch into \a [cmtype ? defensive : offensive] fighting stance.</span>")
	...
	switch (severity)
		if(1.0)
			if(cmtype == CM_DEF && prob(5+cmrate))
				adjustBruteLoss(100)
				to_chat(src, "<span class='warning'>You manage to partially block the intense blastwave!</span>")
				cmrate = 5
			else
				adjustBruteLoss(300)
				to_chat(src, "<span class='danger'>The explosion disrupts your physical manifestation!</span>")

		if(2.0)
			if(cmtype == CM_DEF && prob(45+cmrate))
				adjustBruteLoss(25)
				to_chat(src, "<span class='warning'>You manage to partially block the blastwave!</span>")
				cmrate = 5
			else
				adjustBruteLoss(60)
				to_chat(src, "<span class='danger'>The explosion greatly disrupts your physical manifestation!</span>")
		...
		if(3.0)
			if(cmtype == CM_DEF && prob(65+cmrate))
				...
				adjustBruteLoss(30)
				to_chat(src, "<span class='danger'>The explosion slightly disrupts your physical manifestation!</span>")
