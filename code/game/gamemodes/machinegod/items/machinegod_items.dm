...
/obj/item/weapon/spear/clockspear/attack(var/mob/target, mob/living/user )
	...
	if(iscultist(user))
		user.Paralyse(5)
		to_chat(user, "<span class='warning'>An unexplicable force powerfully repels the spear from [target]!</span>")
		to_chat(user, "<span class='clockwork'>\"You're liable to put your eye out like that.\"</span>")
		if(ishuman(user))
