...
/obj/item/clothing/glasses/wraithspecs/OnMobLife(var/mob/living/carbon/human/wearer)
	...
	if(E && wearer.glasses == src)
		...
		if(E.damage >= E.min_broken_damage && !(wearer.sdisabilities & BLIND))
			to_chat(wearer, "<span class='danger'>You go blind!</span>")
			wearer.sdisabilities |= BLIND
		else if (E.damage >= E.min_bruised_damage && !(wearer.disabilities & NEARSIGHTED))
			to_chat(wearer, "<span class='danger'>You're going blind!</span>")
			wearer.eye_blurry = 5
			wearer.disabilities |= NEARSIGHTED
		if(prob(15))
			to_chat(wearer, "<span class='danger'>Your eyes burn as you look through the spectacles.</span>")
...
/obj/item/clothing/glasses/wraithspecs/equipped(var/mob/M, glasses)
	...
	if(H.glasses == src)
		...
		if(!(H.sdisabilities & BLIND))
			if(iscultist(H))
				to_chat(H, "<span class='clockwork'>\"Looks like Nar'sie's dogs really don't value their eyes.\"</span>")
				E.damage += E.min_broken_damage
				to_chat(H, "<span class='danger'>You go blind!</span>")
				H.sdisabilities |= BLIND
				return

			to_chat(H, "<span class='clockwork'>Your vision expands, but your eyes begin to burn.</span>")
			E.damage += 4
			...
			if(E.damage >= E.min_broken_damage && !(H.sdisabilities & BLIND))
				...
				to_chat(H, "<span class='danger'>You're going blind!</span>")
				H.eye_blurry = 5
				H.disabilities |= NEARSIGHTED
		else
			to_chat(H, "<span class='clockwork'>\"You're already blind, fool. Stop embarassing yourself.\"</span>")
			return
