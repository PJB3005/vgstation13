...
/obj/item/clockslab/New()
	...
	components = global.CLOCK_COMP_STORAGE_PRESET.Copy()
...
/obj/item/clockslab/preattack(var/atom/target, var/mob/user, var/proximity_flag, var/click_parameters)
	...
	if (isclockcult(M))
		to_chat(user, "<span class='clockwork'>[M] is already a follower!</span>")
		return 1
	else
	...
	if (is_convertable_to_cult(M.mind) && !jobban_isbanned(M, "cultist"))//putting jobban check here because is_convertable uses mind as argument
		...
		to_chat(M, "<span class='clockwork'>Your mind is racing! Your body feels incredibly light! Your world glows a brilliant yellow! All at once everything connects to you. The clockwork justiciar lies in exile, derelict and forgotten in an unseen realm.</span>")
		to_chat(M, "<span class='clockwork'>Assist your new compatriots in their righteous efforts. Their goal is yours, and yours is theirs. You serve the Justiciar above all else. Bring Him back.</span>")
		...
		M.Weaken(30) // Weaken works with mob ticks so 1 mob tick: 2 seconds.
		M.silent += 60
...
/obj/item/clockslab/examine(mob/user)
	..()
	if (isclockcult(user))
		to_chat(user, "The word of Ratvar, The One Who Judges, The Clockwork Justiciar. Contains the details of all the powers his followers can possibly call upon. Without the Justiciar's presence, they're all weakened, however.")
	else
		to_chat(user, "...and not a word of it makes any sense to you.")
...
/obj/item/clockslab/attack_self(mob/living/user as mob)
	...
	if (iscultist(user))	//Cultists be scared to shit.
		to_chat(user, "<span class='sinister'>You reek of blood. You�ve got a lot of nerve to even look at that slab.</span>")
		return 1
...
/obj/item/clockslab/Topic(var/href, var/list/href_list)
	...
	if (!isclockcult(usr))
		to_chat(usr, "<span class='warning'>You don't even have any idea what any of this means, better not touch it...</span>")
		return 1
